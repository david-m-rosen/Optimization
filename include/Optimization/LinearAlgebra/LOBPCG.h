...
/** The following (commented) code block implements the robust LOBPCG method
* described in the paper "A Robust and Efficient Implementation of LOBPCG", by
...
LOBPCG(const SymmetricLinearOperator<Matrix, Args...> &A,
  ...
  for (num_iters = 0; num_iters < max_iters; ++num_iters) {
    ...
    if (useOrtho && !useOrtho_prev) {
      ...
      std::cout << "useOrtho switched on!" << std::endl << std::endl;
    ...
    // for user-defined stopping criterion
    if (user_function &&
        (*user_function)(num_iters, A, B, T, nev, Thetax, X, r, nc, args...))
      break;
